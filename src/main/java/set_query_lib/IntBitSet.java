...
public class IntBitSet implements Comparable<IntBitSet>{
  ...
  /**
   * How many bits in "local" do not occur in "otherBst"
   */
  public static int localDistance(IntBitSet local, IntBitSet otherBst){

    int i = 0;
    int j = 0;

    int overlap = 0;
    while(i < local.contents.length && j < otherBst.contents.length){

      int val = local.contents[i];
      int oval = otherBst.contents[j];
    ...
    return (local.contents.length - overlap);
  }

  public double weight(double[] weights){
    double value = 0;
  ...
  public double weight(List<Double> weights){
    double value = 0;
    for (int content : this.contents) {
      value += weights.get(content);
    }

    return value;
  }

  public static double weightedLocalDistance(IntBitSet bstA, IntBitSet bstB, double[] cpms){
    return weightedLocalDistance(bstA, 1, bstB, cpms);
  }


  /**
   * Version of localDistance where bits are given weights
   */
  public static double weightedLocalDistance(IntBitSet subject, long countA, IntBitSet otherBst, double[] weights){

    int i = 0;
    int j = 0;

    double dropcost = 0;
    while(i < subject.contents.length && j < otherBst.contents.length){

      int val = subject.contents[i];
      int oval = otherBst.contents[j];

      if(val < oval){
        dropcost+=(weights[subject.contents[i]]*countA);
        i++;
      }else if(val > oval){
        j++;
      }else{
  ...
  public static double weightedLocalDistance(IntBitSet subject, long countA, IntBitSet otherBst, List<Double> weights){
    ...
    while(i < subject.contents.length && j < otherBst.contents.length){
      ...
      if(val < oval){
        dropcost+=(weights.get(subject.contents[i])*countA);
        i++;
      }else if(val > oval){
        j++;
      }else{
  ...
  public static double weightedDistance(IntBitSet bstA, IntBitSet bstB, double[] cpms){
    return weightedDistance(bstA, 1, bstB, 1, cpms);
  }

  public static double weightedDistance(IntBitSet bstA, long sizeA, IntBitSet bstB, long sizeB, double[] cpms){

    int i = 0;
    int j = 0;

    double dropcost = 0;
    while(i < bstA.contents.length && j < bstB.contents.length){

      int val = bstA.contents[i];
      int oval = bstB.contents[j];

      if(val < oval){
        dropcost+=(cpms[bstA.contents[i]]*sizeA);
        i++;
      }else if(val > oval){
        dropcost+=(cpms[bstB.contents[j]]*sizeB);
        j++;
      }else{
        i++;
        j++;
    ...
    while(j < bstB.contents.length){
      dropcost+=(cpms[bstB.contents[j]]*sizeB);
      j++;
    }

    while(i < bstA.contents.length){
      dropcost+=(cpms[bstA.contents[i]]*sizeA);
      i++;
    }
